...
switch ($_POST['form_id']){
    ...
    case 'modifier_utilisateur_admin':
            $update = new Update('banner', 'Sur Esotral', $pdo_cnx);
            $update->set_update(['Nom', 'Prenom', 'Email'], 'User', 'ID = "'.$_POST['ID'].'"');
        exit;
        break;

    case 'modifier_produit':
            $update = new Update('banner', 'Sur Esotral', $pdo_cnx);
            $update->set_update(['Nom', 'Description', 'Prix', 'Model', 'MotsCles', 'Qte'], 'Produits', 'ID = "'.$_POST['ID'].'"');
        exit;
        break;
