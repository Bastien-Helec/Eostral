    ...
    */ 
    foreach ($formData['elements'] as $element) {
        ...
        if (isset($element['nom'])) {
            ...
            // On verifie si le champs est requis ou non
            $requis = (isset($element['requis']) && $element['requis'] == true) ? 'required' : '';

            // On verifie si le champs est un type 'number'

            if ($element['type'] == 'number') {

                echo '<input type= "'.$element['type']. '" min="1" value="1" name= "'. $element['nom']. '" id= "'.$formData['id']. '_'. $element['nom']. '" placeholder= "'.$element['placeholder']. '" '.$requis.'>';

            } else {
            // On créer donc le champ associé
            echo '<input type= "'.$element['type']. '" name= "'. $element['nom']. '" id= "'.$formData['id']. '_'. $element['nom']. '" placeholder= "'.$element['placeholder']. '" '.$requis.'>';
        }
        }
        ...
        elseif (isset($element['nom_liste'])) {
            ...
            // On va chercher les libelles de la BDD corespondante au valeur des colonnes
